...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a DecisionTree educational report.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "runtime" / "reports" / "decision_tree_example",
    )
...
def create_report_example(*, output_dir: Path | str, random_state: int = 42) -> tuple[Path, Path]:
    ...
    model = DecisionTreeClassifier(max_depth=4, random_state=random_state).fit(X_train, y_train)
    return (
        report_for(model)
        .with_training_data(
            X_train=X_train,
        ...
        .save(output_dir, stem="decision_tree_report")
    )
