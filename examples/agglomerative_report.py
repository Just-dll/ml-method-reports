...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate an AgglomerativeClustering educational report."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "runtime" / "reports" / "agglomerative_example",
    )
...
def create_report_example(*, output_dir: Path | str, random_state: int = 42) -> tuple[Path, Path]:
    X, _ = make_blobs(
        n_samples=120, n_features=len(FEATURE_NAMES), centers=3, random_state=random_state
    )
    dataframe = pd.DataFrame(X, columns=FEATURE_NAMES)
    X_scaled, _, scaling_params = standard_scale_frames(dataframe, dataframe)
    model = AgglomerativeClustering(n_clusters=3, compute_distances=True).fit(X_scaled)
    return (
        report_for(model)
        ...
        .save(output_dir, stem="agglomerative_report")
    )
