...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a generic classification report.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "runtime" / "reports" / "generic_classification_example",
    )
...
def create_report_example(*, output_dir: Path | str, random_state: int = 42) -> tuple[Path, Path]:
    ...
    return (
        ...
        .with_metadata(example="generic_classification_report")
        .as_generic()
        .save(output_dir, stem="generic_classification_report")
    )
